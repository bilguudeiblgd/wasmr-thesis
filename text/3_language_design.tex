...
Functions that modify or read a variable outside its scope in the typed R-like language are \textbf{closures}: they capture the environment in which they are defined. When a function is called:
\begin{enumerate}
...
The type system ensures type safety through static analysis while supporting type inference to maintain concise syntax. Type inference fails for programs lacking sufficient type information, requiring explicit annotations. The type grammar defines three major categories:

\begin{align*}
\tau ::= & \; num \mid \texttt{void} && \text{(base type)} \\
       | & \; \tau\texttt{[]} && \text{(vector type)} \\
       | & \; (\tau_1, \ldots, \tau_n) \to \tau && \text{(function type)} \\[0.5em]
num    ::= & \; \texttt{int} \mid \texttt{double} \mid \texttt{logical} && \text{(numeric types)}
\end{align*}
...
We do not formalize the complete type inference algorithm here, as bidirectional type inference is well-studied in programming language literature\cite{bidirectional_typing}. Our implementation follows standard techniques, extended with the subtyping hierarchy.

\subsection{Selected Typing Rules}

This subsection presents key typing rules that formalize crucial aspects of the type system.

Typing judgments have the form $\Gamma \vdash e : \tau$, read as "under typing context $\Gamma$, expression $e$ has type $\tau$." A typing context $\Gamma$ maps variable names to their types. Rules appear in natural deduction style with premises above the horizontal line and conclusions below.

The \textbf{subsumption rule} connects subtyping to the type system, enabling implicit type promotion:
...
\noindent
\textbf{Binary expression}: If expression $e$ has type $\tau_1$ and $\tau_1$ is a subtype of $\tau_2$, then $e$ can also be given type $\tau_2$, enabling all implicit conversions. For example, if \texttt{x} has type \texttt{int}, we can use it where \texttt{double} is expected.

Binary operators extend component-wise to vectors when both operands have the same numeric vector type:
...
where $\oplus \in \{+, -, *, /, \texttt{\%\%}\}$. For example, \texttt{c(1,2) + c(3,4)} produces \texttt{c(4,6)}.

\noindent
\textbf{Function definition and application}: To type-check a function, we extend the context with parameter bindings and verify the body has the declared return type:
...
The result has the function's return type.

\noindent
\textbf{Closures and Super-assignment}: Super-assignment modifies a variable in an enclosing scope, crucial for implementing closures:
...
where $\tau' <: \tau$ (the assigned value must be compatible with the existing binding). Super-assignment requires that $x$ already exists in some enclosing function scope ($\Gamma_{outer}$), enabling closures to modify captured variables. This is essential for stateful closures like counters.

For this section, I showed what are the important parts 
typing rules. The complete rules are not shown as they are repetitive and general.
