...
\node[scalarnode] (logical) at (-3, 4) {logical};
\node[scalarnode] (int) at (0, 4) {int};
\node[scalarnode] (double) at (3, 4) {double};
...
% Annotation boxes
\node[draw, rounded corners, fill=yellow!10, align=left, font=\scriptsize, text width=3.5cm] at (-6.5, 3.2) {
...
% Legend
\node[font=\scriptsize\bfseries, anchor=west] at (-6.5, -1.2) {Legend:};
\draw[subtype] (-6.5, -1.6) -- (-5.5, -1.6) node[anchor=west, font=\scriptsize] {Scalar subtyping};
\draw[vecsubtype] (-6.5, -2.0) -- (-5.5, -2.0) node[anchor=west, font=\scriptsize] {Vector subtyping};
\draw[crosssubtype] (-6.5, -2.4) -- (-5.5, -2.4) node[anchor=west, font=\scriptsize] {Scalar-to-vector};
