...
Due to this thesis' primarily aim being compiling a dynamic language like R to 
WASM, it's important to specify clearly to what degree and which subset to choose.
A full featureset of R will be notoriously hard to compile.
...
\begin{align*}
\langle\textit{program}\rangle \; ::= \; & \langle\textit{statement}\rangle^* \\[0.5em]
%
\langle\textit{statement}\rangle \; ::= \; & \langle\textit{assignment}\rangle \mid \langle\textit{control flow}\rangle \mid \langle\textit{expression}\rangle \mid \langle\textit{index assign}\rangle \mid \langle\textit{return}\rangle \\[0.5em]
%
\langle\textit{control flow}\rangle \; ::= \; & \langle\textit{if}\rangle \mid \langle\textit{for}\rangle \mid \langle\textit{while}\rangle \\[0.5em]
%
\langle\textit{if}\rangle \; ::= \; & \texttt{if (} \langle\textit{expression}\rangle \texttt{) \{} \langle\textit{statements}\rangle \texttt{\}} \\
& \mid \; \texttt{if (} \langle\textit{expression}\rangle \texttt{) \{} \langle\textit{statements}\rangle \texttt{\} else \{} \langle\textit{statements}\rangle \texttt{\}} \\[0.5em]
%
\langle\textit{for}\rangle \; ::= \; & \texttt{for (} \langle\textit{identifier}\rangle \texttt{ in } \langle\textit{expression}\rangle \texttt{) \{} \langle\textit{statements}\rangle \texttt{\}} \\[0.5em]
%
\langle\textit{assignment}\rangle \; ::= \; & \langle\textit{identifier}\rangle \; [\texttt{:} \langle\textit{type}\rangle] \; \texttt{<-} \; \langle\textit{expression}\rangle \\[0.5em]
%
\langle\textit{expression}\rangle \; ::= \; & \langle\textit{literal}\rangle \mid \langle\textit{identifier}\rangle \mid \langle\textit{function\_def}\rangle \mid \langle\textit{function\_call}\rangle \mid \ldots \\[0.5em]
%
\langle\textit{function\_def}\rangle \; ::= \; & \texttt{function (} \langle\textit{param\_list}\rangle \texttt{) :} \langle\textit{type}\rangle \texttt{ \{} \langle\textit{statement}\rangle^* \texttt{\}} \\[0.5em]
%
\langle\textit{param\_list}\rangle \; ::= \; & \langle\textit{param}\rangle \; (\texttt{,} \langle\textit{param}\rangle)^* \\[0.5em]
%
\langle\textit{param}\rangle \; ::= \; & \langle\textit{identifier}\rangle \texttt{:} \langle\textit{type}\rangle \\[0.5em]
%
\langle\textit{function\_call}\rangle \; ::= \; & \langle\textit{identifier}\rangle \texttt{(} \langle\textit{arg\_list}\rangle \texttt{)} \\[0.5em]
%
\langle\textit{function\_call}\rangle \; ::= \; & \langle\textit{identifier}\rangle \texttt{(} \langle\textit{arg\_list}\rangle \texttt{)} \\[0.5em]
%
\langle\textit{arg\_list}\rangle \; ::= \; & \langle\textit{expression}\rangle \; (\texttt{,} \langle\textit{expression}\rangle)^* \\[0.5em]
%
\langle\textit{type}\rangle \; ::= \; & \texttt{integer} \mid \texttt{logical} \mid \texttt{double} \mid \texttt{(} \langle\textit{type}\rangle \texttt{ -> } \langle\textit{type}\rangle \texttt{)} \\
& \mid \; \texttt{any} \mid \texttt{function} \mid \langle\textit{composite type}\rangle \\[0.5em]
%
\langle\textit{composite type}\rangle \; ::= \; & \texttt{vector} \; \langle\textit{type}\rangle
\end{align*}
...
\begin{itemize}
    ...
    \item \textbf{Type keywords:} \texttt{int}, \texttt{double}, \texttt{void}, \texttt{logical}, \texttt{any}, \texttt{vector}, \texttt{list}
...
\noindent The language supports various expression forms:
