...
\subsubsection{Literals and Variables}
...
\textbf{Explanation:} All elements must have the same type $\tau$ (after promotion). The result is a vector parameterized by that element type. For example, \texttt{c(1, 2, 3)} has type \texttt{vector<int>}.
...
\inferrule*[right=T-Range]
    {\Gamma \vdash e_1 : \texttt{int} \\ \Gamma \vdash e_2 : \texttt{int}}
    {\Gamma \vdash e_1\texttt{:}e_2 : \texttt{vector}\langle\texttt{int}\rangle}
\end{mathpar}

\textbf{Explanation:} The range \texttt{1:5} creates the vector \texttt{c(1, 2, 3, 4, 5)}. Both bounds must be integers.
...
\inferrule*[right=T-Index]
    {\Gamma \vdash e_1 : \texttt{vector}\langle\tau\rangle \\ \Gamma \vdash e_2 : \texttt{int}}
    {\Gamma \vdash e_1[e_2] : \tau}
\end{mathpar}

\textbf{Explanation:} Indexing a vector of type \texttt{vector<$\tau$>} with an integer index produces a value of type $\tau$. Note that our language uses 1-based indexing, following R convention.
...
While loops do not produce meaningful values:

\begin{mathpar}
\inferrule*[right=T-While]
    {\Gamma \vdash e_1 : \texttt{logical} \\ \Gamma \vdash e_2 : \tau}
    {\Gamma \vdash \texttt{while}\ (e_1)\ \{e_2\} : \texttt{void}}
\end{mathpar}

\textbf{Explanation:} The condition must be logical. The body is checked but its type is discarded; the loop itself has type \texttt{void}.

\paragraph{For Loops}
For loops iterate over vectors:

\begin{mathpar}
\inferrule*[right=T-For]
    {\Gamma \vdash e_1 : \texttt{vector}\langle\tau_1\rangle \\ \Gamma, x:\tau_1 \vdash e_2 : \tau_2}
    {\Gamma \vdash \texttt{for}\ (x\ \texttt{in}\ e_1)\ \{e_2\} : \texttt{void}}
\end{mathpar}

\textbf{Explanation:} The iterable must be a vector. The loop variable $x$ is bound to the element type within the body. Like while loops, for loops have type \texttt{void}.
...
This closure semantics enables patterns like factory functions and stateful callbacks, as shown in the counter example in Section~\ref{sec:scoping}.
