...
% )
% \end{lstlisting}
After variable assignment, we have very interesting syntax for 
super assignment. R distinguishes between assigning a variable
in the scope or outside the scope with different syntax, unlike
other popular dynamic languages like Python or Javascript. In Typed R,
we'll do the same. 
...
\end{lstlisting}
I should also note that on Code fragment~\ref{r_superassignment} above, Typed R looks
exactly same as R. For trivial cases like this, type inference can help a lot, which we will talk in next chapter.
...
\end{itemize}
Given \texttt{int -> int -> int}, for example, the grammar is ambiguous. Therefore, in our syntax, we use parenthesis so that it's well-defined. 
...
The type checker performs bidirectional type inference\cite{bidirectional_typing}. \textbf{Bottom-up inference} infers expression types from literal values and operator signatures, while \textbf{top-down checking} uses function return types and variable annotations to provide expected types. \textbf{Unification} solves type constraints to determine concrete types, incorporating subtyping rules to allow implicit type promotion.
...
\subsubsection{Reflexivity and Base Cases}
...
\subsubsection{Vector Covariance}
...
\subsubsection{Range Construction}
...
\subsubsection{Scalar-Vector Operations}
...
\subsubsection{Super-Assignment}
