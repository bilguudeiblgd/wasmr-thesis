...
Runtime is divided into two parts. One is being standard-like functions written in Typed R. The other is functions built with help of
the compiler. For example, a function \texttt{print}, is impossible to write as an R program without any knowledge of what is it in compiler. First, let's talk
about runtimes ported as R, one can say, it's a standard library. It's easier to write Typed R code than WASM. For mathematical and statistics operations,
coding them as Typed R, and compiling it together is convenient and also helps to find bugs. Below Figure~\ref{fig:runtime-compilation} shows the diagram of such process.
\pagebreak
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    node distance=1cm and 1.8cm,
    box/.style={rectangle, draw, fill=blue!10, minimum width=2.5cm, minimum height=0.8cm, align=center, rounded corners, font=\small},
    inputbox/.style={rectangle, draw, fill=cyan!20, minimum width=2.5cm, minimum height=0.8cm, align=center, rounded corners, font=\small},
    mergebox/.style={rectangle, draw, fill=orange!20, minimum width=2.5cm, minimum height=0.8cm, align=center, rounded corners, font=\small},
    arrow/.style={->, >=stealth, thick}
]

% Left path - User source
\node[inputbox] (user_input) {User R\\Source Code};
\node[box, below=of user_input] (user_ast) {AST};

% Right path - Runtime
\node[inputbox, right=of user_input] (runtime_input) {Runtime R\\Files};
\node[box, below=of runtime_input] (runtime_ast) {AST};

% Merge and pipeline
\node[mergebox, below=1.2cm of user_ast, xshift=2.1cm] (merge) {AST Merge};
\node[box, below=of merge] (ir) {Typed IR};
\node[box, below=of ir] (codegen) {Code Generator};
\node[font=\small\bfseries, below=of codegen] (output) {Single WASM};

% Arrows
\draw[arrow] (user_input) -- (user_ast);
\draw[arrow] (runtime_input) -- (runtime_ast);
\draw[arrow] (user_ast) -- (merge);
\draw[arrow] (runtime_ast) -- (merge);
\draw[arrow] (merge) -- (ir);
\draw[arrow] (ir) -- (codegen);
\draw[arrow] (codegen) -- (output);

\end{tikzpicture}
\caption{Simplified compilation pipeline: user source code and runtime files are parsed into separate ASTs, merged, type-resolved into IR, and compiled to a single WebAssembly module.}
\label{fig:runtime-compilation}
\end{figure}

However, there's inherent limitation to write a runtime with Typed R. It won't be able to express low-level details. Therefore, we also have built-in functions which serves as runtime. A good example is \texttt{print}. It's provided by WASI integration, which provides I/O capabilities through the WebAssembly System Interface. The runtime imports \texttt{fd\_write} from \texttt{wasi\_snapshot\_preview1} with signature \texttt{(i32, i32, i32, i32) -> i32}, accepting parameters for file descriptor, iovs pointer, iovs length, and nwritten pointer. This enables output to stdout and stderr.

Moreover, the runtime uses WebAssembly GC for automatic memory management, treating vectors, and closures as GC-managed heap objects. This eliminates the need for explicit deallocation, as WebAssembly GC handles reference counting and garbage collection automatically. Linear memory is reserved for WASI I/O buffers and string serialization. The memory layout reserves address 0 for null pointer checks, allocates addresses from 8 onward for WASI I/O buffers used by \texttt{fd\_write}, and maintains a dynamic region managed by the compiler for temporary string buffers.

\section{Implementation Details}

\subsection{Compilation Limitations}

The current implementation represents a proof-of-concept compiler with several deliberate limitations. Most notably, the compiler lacks any exception or error handling mechanism, meaning runtime errors simply terminate execution without graceful recovery. The standard library remains minimal, providing only essential built-in functions like \texttt{print}, \texttt{length}, \texttt{sum}, and vector construction. I/O capabilities are restricted to print output via WASI; file operations and user input are not supported. Type coercion is limited compared to R's flexible type system, supporting only the basic numeric promotion hierarchy (logical $<:$ int $<:$ double). Vector operations cover common arithmetic and indexing but lack many of R's specialized functions like \texttt{apply}, \texttt{sapply}, or statistical operations. 

\subsection{Design Decisions}
Several architectural decisions shaped the compiler's implementation, each involving trade-offs between performance, simplicity, and compatibility with R semantics. The choice of static typing fundamentally enables efficient ahead-of-time compilation and early error detection, though at the cost of R's dynamic flexibility. 

One of the most important design principle was to only emit single WASM file. It's choice between single WASM file or module WASM files, where one could represent the runtime and other 
user module compilation. Then these multiple modules would talk to each other through imports and such. This seems well enough but practically, linking of WASM is not that simple. It needs an 
environment and some language, which has or connected to some WASM runtime and its SDK so where they can be linked together. This brings a lot of friction when it comes to using our compiler.
Therefore, for sake of convenience when executing we only emit single WASM file. It allows runtimes to easily execute the generated WASM code from Typed R. Although it's biggest trade-off is that,
every WASM file we generate contains the runtime. So runtime functions explode, the file size can explode as well. Moreover, it makes debugging very hard. It's hard for users to find their function, swimming through
big chunk of code generated for runtime. With all said, it's still the simplest and effective way. 
...
Local variable allocation reuses slots for temporary variables when their lifetimes don't overlap, minimizing the function's local declaration section and potentially improving WebAssembly JIT compilation. 
The compiler also caches function signature type indices to avoid recreating identical types in the WebAssembly type section, reducing module size and improving instantiation time. These optimizations focus on low-hanging fruit that provide measurable benefits without requiring complex analysis passes.
